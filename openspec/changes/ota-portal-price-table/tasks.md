...
- [ ] 1.2 Add `pricing_portal_reference_nights` and
...
- [ ] 1.3 Create `app/Services/OtaPortalNightlyRateService.php` with a method
  `ratesFor(PricingRule $rule): array` returning, for each of
  `airbnb`/`booking`/`hometogo`: the suggested nightly rate in cents.
- [ ] 1.4 Implement the formula: read `referenceNights`/`referenceGuests` from
...
- [ ] 2.1 Extend `SettingsController::updatePricing()` validation to accept
  `pricing_portal_reference_nights` (integer, min 1) and
  `pricing_portal_reference_guests` (integer, min 1), persisting via
  `Setting::set()`.
- [ ] 2.2 Extend `SettingsController::pricingSettings()` (or equivalent view
  data helper) to expose the current/default values of both new settings.
- [ ] 2.3 Add the 2 new fields to the "Fiscalità e prezzi" card in
...
- [ ] 4.1 Create `resources/views/admin/pricing/portal-prices.blade.php`: a
  ...
  euros.
- [ ] 4.2 Add a link to this new page from `admin/prezzi/index.blade.php`
  (next to the existing "Formula prezzi" link) and from the "Formula prezzi"
  page, so it's discoverable from both.

## 5. Tests

- [ ] 5.1 Add unit tests for `OtaPortalNightlyRateService::ratesFor()`
  ...
  the reference total.
- [ ] 5.2 Add a feature test for the new `admin/prezzi-portali` route:
...
- [ ] 5.3 Add a feature test for `SettingsController::updatePricing()`
  persisting the 2 new reference settings and validation errors for
  non-positive values.

## 6. Docs

- [ ] 6.1 Update `docs/specific-tech-backend-doc.mdc`'s `PricingQuoteService`
  / `OtaPortalPricingService` note to mention the new
  `OtaPortalNightlyRateService` and the 2 new `pricing_portal_reference_*`
  settings.
- [ ] 6.2 Update `docs/business-doc.mdc` admin flows section to mention the
  new "Prezzi portali" page alongside the existing pricing simulator note.

## 7. Verification

- [ ] 7.1 Run the full test suite and confirm all tests pass.
- [ ] 7.2 Manually check `admin/prezzi-portali` shows sensible, close-to-equal
