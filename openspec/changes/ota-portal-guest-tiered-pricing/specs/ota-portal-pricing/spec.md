...
(base nightly rate, extra-guest surcharge, cleaning fee) and the owner's
approximate net revenue after that portal's commission, reusing the same
base-nightly-rate formula as the `ota-portal-price-table` capability rather
than grossing up the already-computed direct total.
...
The admin pricing simulator SHALL display, alongside the existing
direct-price breakdown, each portal's guest-facing total, the owner's
approximate net revenue, and an indicator comparing that net revenue to the
direct-site total for the same simulated stay.
