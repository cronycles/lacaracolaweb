...
- [x] 1.1 Add `pricing_portal_extra_margin_percent` (default `'0.05'`) to
      `SettingsController::PRICING_SETTING_DEFAULTS`.
- [x] 1.2 Add validation (`numeric, min:0, max:100`) and persistence
      (`/ 100` before `Setting::set`, same pattern as `pricing_commission_*`)
      in `updatePricing()`.
- [x] 1.3 Expose the current/default value from `pricingSettings()` as
      `portal_extra_margin_percent`.

## 2. Settings UI

- [x] 2.1 In `resources/views/admin/settings.blade.php`, add a new field to
...
- [x] 3.1 Add a private `extraMarginRate(): float` helper reading
      `Setting::get('pricing_portal_extra_margin_percent', '0.05')`.
- [x] 3.2 `baseNightlyRateCents()`: multiply the commission-grossed rate by
      `(1 + extraMarginRate())` before the nearest-euro rounding.
- [x] 3.3 `guestFacingTotal()`: multiply `baseStayGrossedCents` by
...
- [x] 4.1 Update `tests/Unit/OtaPortalPricingServiceTest.php`'s existing
      worked-example assertions for the new default 5% margin.
- [x] 4.2 Add a test setting `pricing_portal_extra_margin_percent` to a
      non-default value (e.g. `10%`) and asserting both `baseNightlyRateCents()`
      and `guestFacingTotal()` reflect it.
- [x] 4.3 Add a test setting the margin to `0` and asserting the result
      matches the pre-change formula exactly (no regression when disabled).

## 5. Docs

- [x] 5.1 Update the `OtaPortalPricingService` summary in
