...
- [ ] 1.1 Add `pricing_tax_rate`, `pricing_tax_gross_up_items`,
...
- [ ] 1.2 Add `SettingsController::updatePricing(Request $request)` validating the
...
- [ ] 2.2 In `calculate()`, after computing the nightly-stay sum (rename the
...
- [ ] 2.3 After computing `cleaningCents`/`linenCents`/`parkingCents`, compute
...
- [ ] 2.4 Compute `rawTotalCents = discountedStayCents + taxGrossUpCents + cleaningCents + linenCents`.
- [ ] 2.5 Round `rawTotalCents` to the nearest 500 cents; compute the rounding
  delta and fold it into `stayCents = discountedStayCents + taxGrossUpCents + roundingDelta`.
- [ ] 2.6 Update the returned array: keep `stay_cents`/`total_cents`/
...
- [ ] 2.7 Update the PHPDoc `@return` annotations on `calculate()` and
  `unavailable()` to list the new fields.

## 3. Portal price suggestions (`OtaPortalPricingService`)

- [ ] 3.1 Create `app/Services/OtaPortalPricingService.php` with a method
  ...
  documented defaults.
- [ ] 3.2 Reuse the same weekly/monthly discount lookup as `PricingQuoteService`
...
- [ ] 4.1 Inject `OtaPortalPricingService` into
...
- [ ] 5.1 Add a "Fiscalità e prezzi" card to `resources/views/admin/settings.blade.php`
...
- [ ] 5.2 Update `resources/views/admin/pricing/index.blade.php` simulator result
...
- [ ] 5.3 Update `resources/ts/components/pricing-simulator.ts` `SimulationResponse`
...
- [ ] 5.4 Update `resources/views/admin/stay-discounts/index.blade.php` ("Formula
...
- [ ] 6.1 Delete `docs/commissioni portali.md` (content preserved in
  `openspec/changes/tax-gross-up-pricing/design.md`).
- [ ] 6.2 Update `docs/specific-tech-backend-doc.mdc`'s `PricingQuoteService`
  ...
  rounding.
- [ ] 6.3 Update `docs/business-doc.mdc`'s note that `admin/sconti-soggiorno` has
...
- [ ] 7.1 Add unit tests for `PricingQuoteService::calculate()` covering: default
  ...
  the discount.
- [ ] 7.2 Add unit tests for `OtaPortalPricingService::suggest()` covering the 3
...
- [ ] 7.3 Update `BookingLegalConsentTest` and `BookingRequestConfirmationTest`
...
- [ ] 7.4 Add a feature test for `Admin\SettingsController::updatePricing()`
...
- [ ] 8.1 Run the full test suite (`php artisan test` / project's configured
  command) and confirm all pricing-related tests pass with updated expectations.
- [ ] 8.2 Manually verify in the admin pricing simulator that the owner's worked
  example (500 € stay, 200 € cleaning+linen, 21% tax → +42 €, rounded to nearest
  €5) reproduces correctly end to end.
- [ ] 8.3 Manually verify a 7-night and a 28-night simulation show the correct
