...
interface SimulationResponse {
    ...
    portals?: Record<string, PortalGuestTotal>;
    message?: string;
}
...
export function initPricingSimulator(): void {
    ...
    const resultBox = document.querySelector<HTMLElement>(
        "#pricing-sim-result",
    );
    const summaryEl = document.querySelector<HTMLElement>(
        "#pricing-sim-summary",
    );
    const breakdownEl = document.querySelector<HTMLElement>(
        "#pricing-sim-breakdown",
    );
    const discountEl = document.querySelector<HTMLElement>(
        "#pricing-sim-discount",
    );
    const taxEl = document.querySelector<HTMLElement>("#pricing-sim-tax");
    const portalsBox = document.querySelector<HTMLElement>(
        "#pricing-sim-portals",
    );
    const portalsCaptionEl = document.querySelector<HTMLElement>(
        "#pricing-sim-portals-caption",
    );
    ...
    const runSimulation = (): void => {
        ...
        const guests = Math.max(
            1,
            parseInt(guestsInput?.value ?? "2", 10) || 2,
        );
        ...
        const parkingInput =
            form.querySelector<HTMLInputElement>("#sim-parking");
        ...
        fetch(simulateUrl, {
            ...
            .then(async (response) => {
                ...
                if (!response.ok || !data.available) {
                    showError(
                        data.message ??
                            "Simulazione non disponibile per il periodo selezionato.",
                    );
                    return;
                }
                ...
                const parkingDetail =
                    parking > 0
                        ? ` · Parcheggio ${formatCurrency(parking)} da pagare in loco`
                        : "";
                ...
                if (portals) {
                    ...
                    (
                        Object.keys(portalLabels) as Array<
                            keyof typeof portalLabels
                        >
                    ).forEach((key) => {
                        const portalEl = document.querySelector<HTMLElement>(
                            `#pricing-sim-portal-${key}`,
                        );
                        const portal = portals[key];
                        if (portalEl && portal) {
                            const indicator = portal.margin_safe ? "✅" : "⚠️";
                            const indicatorColor = portal.margin_safe
                                ? "#166534"
                                : "#b45309";
                            portalEl.innerHTML = `${formatCurrency(portal.guest_total_cents)} ospite · ${formatCurrency(portal.owner_net_cents)} netto tu · comm. ${(portal.commission_rate * 100).toFixed(1)}% <span style="color:${indicatorColor}">${indicator}</span>`;
                        }
                    });
                    portalsBox.style.display = "grid";
                    if (portalsCaptionEl)
                        portalsCaptionEl.style.display = "block";
                } else {
                    portalsBox.style.display = "none";
                    if (portalsCaptionEl)
                        portalsCaptionEl.style.display = "none";
                }
    ...
    const triggerCheckin = document.getElementById(
        "pricing-sim-trigger-checkin",
    ) as HTMLButtonElement | null;
    const triggerCheckout = document.getElementById(
        "pricing-sim-trigger-checkout",
    ) as HTMLButtonElement | null;

    const guestsInputEl = form.querySelector<HTMLInputElement>("#sim-guests");
    guestsInputEl?.addEventListener("change", runSimulation);
    form.querySelector<HTMLInputElement>("#sim-parking")?.addEventListener(
        "change",
        runSimulation,
    );
